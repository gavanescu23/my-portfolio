...
const SocialLinks = () => {
    ...
    const links = [
        ...
        },
        {
            ...
            href: "/CV - Vlad Gavanescu EP latest.pdf",
            style: "rounded-br-md",
            download: true,
        }
    ]
    ...
    <div className='flex flex-col invisible sl:visible top-[35%] left-0 fixed'>
        <ul className='flex flex-col'>
            {links.map(({id, child, href, style, download}) => (
                            <li key={id} 
                            className={`flex justify-between items-center ml-[-70px] hover:ml-[-10px] rounded-md duration-300 mb-2 w-40 h-14 px-4 bg-white style`}>
